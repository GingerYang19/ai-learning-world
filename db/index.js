// xlsx-adapter.js — Store all data in data/data.xlsx
// Sheets: users, saves, progress, placed_blocks, achievements, stats
...
const SCHEMA = {
  ...
  stats:         ['user_id','play_seconds','blocks_placed','blocks_broken','updated_at'],
};
...
// Ensure data is written on process exit
...
function setProgress(userId, courseId) {
  const exists = tables.progress.find(p => Number(p.user_id) === Number(userId) && p.course_id === courseId);
  if (exists) return;
  tables.progress.push({ user_id: userId, course_id: courseId, completed_at: new Date().toISOString() });
  flush();
}
...
function addAchievement(userId, achvId) {
  const exists = tables.achievements.find(a => Number(a.user_id) === Number(userId) && a.achv_id === achvId);
  if (exists) return;
  tables.achievements.push({ user_id: userId, achv_id: achvId, earned_at: new Date().toISOString() });
  flush();
}
...
function getStats(userId) {
  ...
  if (!s) {
    s = { user_id: userId, play_seconds: 0, blocks_placed: 0, blocks_broken: 0, updated_at: new Date().toISOString() };
    tables.stats.push(s);
  }
  return s;
}
function bumpStats(userId, { play_seconds = 0, blocks_placed = 0, blocks_broken = 0 } = {}) {
  const s = getStats(userId);
  s.play_seconds = (Number(s.play_seconds) || 0) + play_seconds;
  s.blocks_placed = (Number(s.blocks_placed) || 0) + blocks_placed;
  s.blocks_broken = (Number(s.blocks_broken) || 0) + blocks_broken;
...
module.exports = {
  ...
  getSave, upsertSave,
  getProgress, setProgress, setProgressBatch,
  getPlacedBlocks, savePlacedBlocks,
  getAchievements, addAchievement, saveAchievementsBatch,
  getStats, bumpStats,
